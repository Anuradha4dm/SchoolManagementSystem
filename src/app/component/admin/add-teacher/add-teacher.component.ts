...
export class AddTeacherComponent implements OnInit {
  ...
  onChange(event) {
    ...
    this.imagePath =
      this.teacherID + '.' + this.selectedFile.name.split('.')[1];
  ...
  onSubmit(value) {
    ...
    for (let i = 0; i < this.subjects.length; i++) {
      if (i == this.subjects.length - 1) subjectList += this.subjects[i];
      else subjectList += this.subjects[i] + ',';
    }
    ...
    this.adminService.addNewTeacher(formdata).subscribe((data) => {
      if (data)
        this.alertService.competeAlert(
          'New Teacher has been added successfully...'
        );
      else this.alertService.errorAlert('Cannot add teacher.Try again later');
    });
  }
}
